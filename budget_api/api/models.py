...
class Origem(models.Model):
    ...
    )  # e.g., Bank Name, Credit Card Name
    descricao = models.TextField(blank=True, null=True)
...
class Lancamento(models.Model):
    ...
    data_upload = models.DateTimeField(
        auto_now_add=True, help_text="Data do upload do OFX/criação do registro"
    )
    valor = models.DecimalField(
        max_digits=10, decimal_places=2, help_text="Valor da transação"
    )
    descricao_original = models.CharField(
        max_length=255, help_text="Descrição original do OFX ou entrada manual"
    )
    ...
    referencia_mes = models.CharField(
        max_length=7, blank=True, null=True, help_text="Mês de referência (AAAA-MM)"
    )  # e.g., 2023-10

    # Campos editáveis na conciliação
    descricao_editada = models.CharField(
        max_length=255,
        blank=True,
        null=True,
    ...
    hash_transacao = models.CharField(
        max_length=64,
        unique=True,
        blank=True,
        null=True,
    ...
    def save(self, *args, **kwargs):
        if (
            not self.hash_transacao and self.descricao_original
        ):  # Ensure there's a description to hash
            ...
            self.hash_transacao = hashlib.sha256(
