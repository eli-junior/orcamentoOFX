...
def import_ofx_data(request, payload: List[LancamentoCreateSchema]):
    ...
    for tx_data in payload:
        try:
            ...
            with (
                transaction.atomic()
            ):  # Ensures save is atomic for IntegrityError handling
                lancamento.save()
            imported_count += 1
        except (
            IntegrityError
        ):  # Catching if hash_transacao (unique=True) collision occurs
            skipped_due_to_duplicate += 1
        except Exception as e:
            # Log more detailed error on the server side here if possible
            error_details.append(
                {"transaction_description": tx_data.descricao_original, "error": str(e)}
            )
    ...
    if (
        imported_count == 0
        and skipped_due_to_duplicate == 0
        and skipped_due_to_error == 0
        and len(payload) > 0
    ):
        ...
        return 200, {
            "message": "Nenhuma nova transação foi importada. Todas as transações fornecidas já existiam ou não puderam ser processadas."
        }

    return 201, {
        "message": f"Importação bem-sucedida. {imported_count} transações importadas."
    }
