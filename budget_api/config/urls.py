...
from django.urls import include, path  # include is used for debug_toolbar
...
@public_api.get(
    ...
    description="Checks if the service is ready to handle requests, including database connectivity.",
)
def readiness_probe(request):
    ...
    try:
        ...
        return JsonResponse(
            {"status": "DOWN", "database": "Error", "details": str(e)}, status=503
        )
